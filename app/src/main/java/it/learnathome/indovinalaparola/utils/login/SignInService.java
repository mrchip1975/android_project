...
public class SignInService extends IntentService {
    ...
    public SignInService() {
        super(SIGN_IN_SERVICE_ID);
    }

    @Override
    protected void onHandleIntent(Intent intent) {
        Intent response = new Intent();
        response.setAction(SIGN_IN_SERVICE_ID);
        try {
            ...
            try(PrintWriter out = new PrintWriter(connection.getOutputStream());) {
                ...
                if(connection.getResponseCode()== HttpURLConnection.HTTP_OK) {
                    ...
                    response.putExtra("response","Error "+connection.getResponseMessage()+ " "+connection.getResponseCode());
                }
            }
        } catch (IOException e) {
            response.putExtra("response","Error SignIn"+e.getMessage());
        } finally {
